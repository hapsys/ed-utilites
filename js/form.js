var proxy = new ProxyApi(false, false, '/');
...
$(function() {
	...
	$('.search-settlements').each(function() {
		...
		var formSubmit = function() {
			...
			proxy.makeCall('post', 'systems/ajax/settlements-search/?' + query, null, null, post, function(result) {
				//console.log(result);
				var table = $('.table-systems');
				$(table).find('.dynamic').remove();
				var i=1;
				$.each(result, function(id, s) {
					...
					$(tr).find("button").on("click", function() {
						...
						if ($(this).hasClass('btn-danger')) {
							...
							delete visited[s.settlement_id]; 
							storage.set(visited);
		...
		$(form).find("#system").ajaxChosen({
			...
			afterTypeDelay: 500,
		    type: 'GET',
		    url: '/systems/ajax/system-search/',
		    dataType: 'json'
		}, function (data) {
			var results = [];
			$.each(data, function (i, val) {
				results.push({ value: val.system_id, text: val.name });
			});
			return results;
		});
		...
		$("#modalVisited").on("show.bs.modal", function() {
			...
			$.each(vsorted, function(id, s) {
				var tr = $('<tr class="modal-dynamic " data-system="'+s.settlement_id+'"><td>'+(id+1)+'</td><td>'+s.name+'</td><td>'+s.title+'</td><td><a href="https://eddb.io/system/'+s.system_id+'" target="_tab">'+s.system_name+'</a></td><td>'+s.planet+'</td><td>'+s.sol_dest+'</td><td>'+s.expire_str+'</td><td class="text-center"><button type="button" class="btn btn-xs"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></button></td></tr>');
				tr.appendTo(table);
			});
